...
class EmployeeTermination(models.Model):
    ...
    def _compute_amounts(self):
        for rec in self:
            ...
            # normal, sin exit_date) y el Simulador -- se pasa
            # exit_date=exit_d para activar el CASO LIQUIDACION: calculo
            # dinamico desde el dia siguiente al corte real del
            # Acumulado Inicial hasta la fecha de salida real (puede
            # terminar antes de noviembre, reflejando que la persona no
            # trabajo el año completo). Confirmado con el usuario que
            # este es el comportamiento correcto para una liquidacion.
            rh_agu = self.env['planilla.rate.helper']
            resultado = rh_agu.calc_aguinaldo_completo(
                emp, exit_year, exit_date=exit_d)
            rec.aguinaldo_months = total_months
            rec.aguinaldo_amount = resultado['total_final']
