"""
Tests v5.12 — Cobertura General: flujos completos, legalidad CR y regresión
============================================================================
Este archivo aumenta la cobertura del módulo hacia el 10/10 cubriendo
flujos y casos de borde que no estaban cubiertos en los tests anteriores.

Áreas cubiertas:
  - Flujos completos de Horas Extras (cálculo + asiento)
  - Incapacidad CCSS + ROP simultáneos
  - Provisiones correctas en todos los modos de pago
  - Pensión alimentaria prioridad sobre embargo (Ley 8590)
  - Préstamo + ROP + embargo en la misma boleta
  - Proporcional (empleado que ingresa a mitad de período)
  - Frecuencias de pago: quincenal, semanal, mensual
  - Acciones de planilla: confirm → pay → reset → cancel
  - Historial salarial automático al confirmar boleta
  - Validaciones de salario mínimo MTSS
  - Planilla con múltiples sucursales
...
# ═══════════════════════════════════════════════════════════════════════
# Frecuencias de pago — factor de conversión
# ═══════════════════════════════════════════════════════════════════════
class TestFrecuenciasPago(TestCoverageBase):
    ...
    def test_01_quincenal_salario_mitad_del_mensual(self):
        ...
        self.assertAlmostEqual(slip.base_salary, expected, delta=100,
            msg=f'Salario quincenal: esperado ₡{expected:,.2f}, obtenido ₡{slip.base_salary:,.2f}')
    ...
    def test_04_periodos_por_mes_bimonthly_es_cero_punto_cinco(self):
        ...
        Un período bimensual ocurre 0.5 veces por mes (cada 2 meses).
        """
        self.assertAlmostEqual(K.PERIODOS_POR_MES['bimonthly'], 0.5, places=3,
            msg='bimonthly debe ser 0.5 períodos/mes (corrección FIX B-04 v58)')


# ═══════════════════════════════════════════════════════════════════════
# Proporcional — ingreso a mitad de período
# ═══════════════════════════════════════════════════════════════════════
class TestProporcional(TestCoverageBase):

    def test_05_salario_proporcional_dias_trabajados(self):
        """
        Empleado que ingresó el día 16 del mes debe recibir 50% del salario
        (15 días de 30 trabajados = factor 0.5).
        """
        emp = self._emp('Prop Ingreso', 600_000)
        # Fecha de ingreso: día 16 del período
        emp.write({'entry_date': '2026-10-16'})
        slip = self._slip(emp, '2026-10-01', '2026-10-31')

        # El cómputo automático debe detectar el ingreso parcial
    ...
    def test_06_factor_proporcional_completo_sin_ingreso_parcial(self):
        """Empleado con período completo tiene factor proporcional = 1.0."""
        emp = self._emp('Prop Completo', 600_000)
        # Fecha de ingreso anterior al período
        emp.write({'entry_date': '2020-01-01'})
        slip = self._slip(emp)
        self.assertEqual(slip.is_proportional, False,
            msg='Empleado con ingreso anterior al período no debe ser proporcional')
        self.assertEqual(slip.proportional_factor, 1.0,
            msg='Factor proporcional debe ser 1.0 para período completo')


# ═══════════════════════════════════════════════════════════════════════
# Horas extras — cálculo y límites legales
# ═══════════════════════════════════════════════════════════════════════
class TestHorasExtras(TestCoverageBase):
    ...
    def test_07_horas_extras_simples_factor_1_5(self):
        ...
        (Art. 139 CT). Verificar que el cálculo de tarifa es correcto.
        """
        from odoo.addons.planilla_cr.models import planilla_const as K
        sal  = 600_000
        ...
        try:
            ...
            self.assertAlmostEqual(slip.overtime_amount, expected_amount, delta=200,
                msg=(
                    f'HE simples 2h: esperado ₡{expected_amount:,.2f}, '
                    f'obtenido ₡{slip.overtime_amount:,.2f}'
                )
            )
        finally:
            he.unlink()
...
class TestProvisiones(TestCoverageBase):
    ...
    def test_09_provision_aguinaldo_8_33_pct(self):
        """Provisión aguinaldo = 8.33% del gross (Art. 228 CT)."""
        ...
        self.assertAlmostEqual(slip.aguinaldo_provision, expected, delta=10,
            msg=f'Provisión aguinaldo: esperado ₡{expected:,.2f}, '
                f'obtenido ₡{slip.aguinaldo_provision:,.2f}')

    def test_10_provision_cesantia_5_33_pct(self):
        """Provisión cesantía = 5.33% del gross (Art. 29 CT)."""
        ...
        self.assertAlmostEqual(slip.cesantia_provision, expected, delta=10,
            msg=f'Provisión cesantía: esperado ₡{expected:,.2f}, '
                f'obtenido ₡{slip.cesantia_provision:,.2f}')

    def test_11_provision_vacaciones_4_16_pct(self):
        """Provisión vacaciones = 4.16% del gross (Art. 153 CT)."""
        ...
        self.assertAlmostEqual(slip.vacation_provision, expected, delta=10,
            msg=f'Provisión vacaciones: esperado ₡{expected:,.2f}, '
                f'obtenido ₡{slip.vacation_provision:,.2f}')
    ...
    def test_12_provisiones_en_asiento_contable(self):
        ...
        self.assertTrue(any('Aguinaldo' in n for n in names),
            msg='Provisión Aguinaldo debe aparecer en el asiento')
        self.assertTrue(any('Cesantía' in n or 'Cesantia' in n for n in names),
            msg='Provisión Cesantía debe aparecer en el asiento')
        self.assertTrue(any('Vacaciones' in n for n in names),
            msg='Provisión Vacaciones debe aparecer en el asiento')


# ═══════════════════════════════════════════════════════════════════════
# Pensión alimentaria — prioridad absoluta sobre embargo
# ═══════════════════════════════════════════════════════════════════════
class TestPensionVsEmbargo(TestCoverageBase):

    def test_13_pension_en_cuenta_separada_230950(self):
        """Pensión alimentaria debe ir a cuenta 230950, no a 230000."""
        ...
        try:
            ...
            self.assertTrue(pen_lines,
                msg='Pensión alimentaria debe aparecer en cuenta 230950')
            self.assertAlmostEqual(
                round(sum(pen_lines.mapped('credit')), 2), 80_000, delta=1,
                msg='Crédito en 230950 debe ser ₡80,000'
            )
    ...
    def test_14_embargo_no_supera_25_pct_neto(self):
        ...
        Un embargo de monto muy alto debe ser recortado al límite legal.
        """
        sal = 500_000
            ...
            if emb_lines:
                ...
                self.assertLessEqual(total_embargo, limite_legal + 0.05,
                    msg=(
                        ...
                        f'  Embargo aplicado: ₡{total_embargo:,.2f}\n'
                        f'  Límite legal 25%: ₡{limite_legal:,.2f}'
                    )
                )
        finally:
            embargo.unlink()
...
class TestFlujoCompletoRun(TestCoverageBase):
    ...
    def test_16_historial_salarial_creado_al_pagar(self):
        """
        Al pagar una boleta debe crearse automáticamente un registro
...
class TestRentaProgresiva(TestCoverageBase):
    ...
    def test_19_salario_bajo_exento_renta_cero(self):
        """Salario por debajo del mínimo exento no paga impuesto de renta."""
        emp  = self._emp('Renta Exento', K.RENTA_EXENTO - 1)
        slip = self._slip(emp)
        self.assertEqual(slip.income_tax, 0.0,
            msg=f'Salario ₡{K.RENTA_EXENTO-1:,} no debe pagar renta (exento ₡{K.RENTA_EXENTO:,})')
    ...
    def test_20_salario_sobre_exento_paga_renta(self):
        ...
        self.assertAlmostEqual(slip.income_tax, expected_renta, delta=500,
            msg=f'Renta al 10%: esperado ≈₡{expected_renta:,.2f}, obtenido ₡{slip.income_tax:,.2f}')

    def test_21_tramos_renta_progresivos(self):
        """Salario alto paga más tramos — verificar que el cálculo es progresivo."""
        ...
        self.assertGreater(slip_a.income_tax, slip_b.income_tax,
            msg='Salario más alto debe pagar más impuesto (renta progresiva)')
...
class TestCCSSTasas(TestCoverageBase):
    ...
    def test_22_ccss_obrero_10_83(self):
        ...
        self.assertAlmostEqual(slip.ccss_employee, expected, delta=1,
            msg=f'CCSS obrero: esperado ₡{expected:,.2f}, obtenido ₡{slip.ccss_employee:,.2f}')
    ...
    def test_23_ccss_patronal_26_83(self):
        ...
        self.assertAlmostEqual(slip.ccss_employer, expected, delta=1,
            msg=f'CCSS patronal: esperado ₡{expected:,.2f}, obtenido ₡{slip.ccss_employer:,.2f}')
...
class TestConstraintUnicidad(TestCoverageBase):
    ...
    def test_25_dos_boletas_mismo_empleado_mismo_periodo_falla(self):
        ...
        # Segunda boleta en el mismo período — debe fallar en el constraint
...
class TestConstantesLegalesCR(TestCoverageBase):
    ...
    def test_27_max_pct_embargo_25(self):
        """Embargo máximo 25% del neto (Art. 172 CT)."""
        self.assertEqual(K.MAX_PCT_EMBARGO, 25.0,
            msg='Embargo máximo debe ser 25% (Art. 172 CT)')

    def test_28_dias_paternidad_8(self):
        """Paternidad: 8 días hábiles (Ley 8107)."""
        self.assertEqual(K.DIAS_PATERNIDAD, 8,
            msg='Días de paternidad deben ser 8 hábiles (Ley 8107)')

    def test_29_dias_vacaciones_por_50_semanas(self):
        """Vacaciones: 12 días por 50 semanas laboradas (Art. 153 CT)."""
        self.assertEqual(K.DIAS_VACACIONES_POR_50_SEMANAS, 12,
            msg='Vacaciones deben ser 12 días/50 semanas (Art. 153 CT)')

    def test_30_meses_prescripcion_vacaciones(self):
        """Prescripción vacaciones: 22 meses sin disfrutarlas (Art. 156 CT)."""
        self.assertEqual(K.MESES_PRESCRIPCION_VACACIONES, 22,
            msg='Prescripción vacaciones = 22 meses (Art. 156 CT)')
    ...
    def test_33_cr_utc_offset_6(self):
        ...
        self.assertEqual(K.CR_UTC_OFFSET_HOURS, 6,
            msg='CR está en UTC-6 (sin horario de verano)')
    ...
    def test_35_tasas_renta_crecientes(self):
        ...
        self.assertEqual(K.RENTA_TASA_4, 0.25,
            msg='Tasa máxima renta 2026 = 25% (DGT-R-016-2026)')
