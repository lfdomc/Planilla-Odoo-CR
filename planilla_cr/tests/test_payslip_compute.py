"""
Tests unitarios -- Calculos de Boleta de Pago (planilla_cr v52 / Odoo 19)

Modelos reales del modulo:
  - planilla.payslip.cr
...
se usa sudo() para evitar restriccion de grupo.
...
class TestPayslipCompute(TransactionCase):
    """Tests para calculos de nomina Costa Rica 2026."""
    ...
    def test_03_ccss_obrero_monto(self):
        """CCSS obrera sobre CRC700,000  CRC75,810."""
        ...
        self.assertAlmostEqual(self.payslip.ccss_employee, expected, delta=2.0,
            msg=f'CCSS obrera esperada CRC{expected:,.2f}')

    def test_04_ccss_patronal_monto(self):
        """CCSS patronal sobre CRC700,000  CRC187,810."""
        self.payslip._compute_deductions()
        expected = round(700_000 * 0.2683, 2)
        self.assertAlmostEqual(self.payslip.ccss_employer, expected, delta=2.0,
            msg=f'CCSS patronal esperada CRC{expected:,.2f}')

    def test_05_renta_exento(self):
        """Salario CRC700,000 esta en tramo exento -- renta = 0."""
        self.payslip._compute_deductions()
        # Tramo exento 2026: hasta CRC941,000
        self.assertEqual(self.payslip.income_tax, 0.0,
            'Salario de CRC700,000 debe estar exento de renta (limite CRC941,000)')

    def test_06_neto_menor_bruto(self):
        """Salario neto < bruto despues de deducciones."""
        self.payslip._compute_deductions()
    ...
    def test_07_aguinaldo_provision(self):
        """Provision aguinaldo = 8.33% del bruto."""
        self.payslip._compute_deductions()
        ...
        self.assertAlmostEqual(self.payslip.aguinaldo_provision, expected, delta=2.0,
            msg='Provision aguinaldo debe ser 8.33%')

    def test_08_cesantia_provision(self):
        """Provision cesantia = 5.33% del bruto."""
        self.payslip._compute_deductions()
        ...
        self.assertAlmostEqual(self.payslip.cesantia_provision, expected, delta=2.0,
            msg='Provision cesantia debe ser 5.33%')

    def test_09_vacaciones_provision(self):
        """Provision vacaciones = 4.16% del bruto."""
        self.payslip._compute_deductions()
        ...
        self.assertAlmostEqual(self.payslip.vacation_provision, expected, delta=2.0,
            msg='Provision vacaciones debe ser 4.16%')

    def test_10_salario_minimo_no_calificado(self):
        """Salario minimo no calificado 2026 >= CRC384,300."""
        min_sal = self.env['planilla.minimum.salary'].search([
            ('active', '=', True),
        ], limit=1)
        if min_sal:
            self.assertGreaterEqual(min_sal.amount, 384_300,
                'Salario minimo no calificado 2026 debe ser >= CRC384,300 (MTSS)')
    ...
    def test_12_feriado_1_octubre_obligatorio(self):
        """1 de octubre (Dia de la Cultura) debe ser feriado de pago obligatorio."""
        from datetime import date
    ...
    def test_13_feriado_2_diciembre_no_obligatorio(self):
        """2 de diciembre (Abolicion Ejercito) debe ser feriado NO obligatorio."""
        from datetime import date
        holiday = self.env['planilla.public.holiday'].search([
            ('date', '=', date(2026, 12, 2)),
        ], limit=1)
...
class TestPayslipCoverageNew04(TransactionCase):
    ...
    def test_14_renta_quincenal_factor_correcto(self):
        ...
        Con salario de CRC800,000 quincenal (CRC19.2M/ano) debe haber retencion.
        """
        ...
        # freq_factor=0.5, y _calc_income_tax multiplica de vuelta x2 para
        ...
        # Para generar retencion se necesita base_salary > CRC941,000 (umbral exento 2026).
        # Usamos CRC2,000,000 mensual -> periodo quincenal = CRC1,000,000 -> renta > 0.
        ...
        # CRC2,000,000 mensual -> equiv. mensual CRC2,000,000 -> supera umbral CRC941,000
        self.assertGreater(
            slip.income_tax, 0,
            'Salario mensual CRC2,000,000 (quincenal) debe generar retencion de renta (umbral exento CRC941,000)'
        )

    # -- TEST: Pension alimentaria + embargo judicial -------------

    def test_15_pension_y_embargo_limite_neto(self):
        """FIX NEW-04 v54 -- Pension alimentaria + embargo judicial:
        la suma de ambas deducciones no puede superar el salario neto
        del empleado (proteccion minimo vital Art. 172 CT).
        """
        slip = self.env['planilla.payslip.cr'].create({
        ...
        if neto > 0 and total_deducciones_especiales > 0:
            self.assertLessEqual(
                ...
                f'Suma pensiones (CRC{pension_total:,.0f}) + embargo (CRC{embargo_total:,.0f}) '
                f'supera neto (CRC{neto:,.0f}). Viola proteccion minimo vital Art. 172 CT.'
            )
    ...
    def test_16_he_feriado_requiere_feriado_registrado(self):
        ...
        # (el martes mas cercano al feriado encontrado)
    ...
    # -- TEST: Liquidacion con prestamo activo -------------------

    def test_17_liquidacion_con_prestamo_activo(self):
        """FIX NEW-04 v54 -- Liquidacion: si el empleado tiene prestamo activo
        con saldo pendiente, el total neto debe ser menor que el bruto (el
        prestamo se descuenta en la liquidacion).
        """
        ...
        # FIX test_17: _compute_all() no existe. Metodos reales: _compute_amounts() + _compute_total()
        term._compute_amounts()
        term._compute_total()

        # Con prestamo activo, el neto debe ser menor que el bruto
        if term.total_gross > 0:
            self.assertLessEqual(
                term.total_net, term.total_gross,
                'Neto liquidacion debe ser <= bruto (prestamo activo debe descontarse)'
            )
    ...
    def test_18_provisiones_suman_correctamente(self):
        ...
        self.assertAlmostEqual(
            ...
            msg=f'Provisiones deben ser ~17.82% del bruto. '
                f'Obtenido: CRC{total_prov:,.2f}, esperado: CRC{expected_amount:,.2f}'
        )
    ...
    def test_19_bono_salarial_suma_al_bruto(self):
        ...
        # Crear codigo de deduccion para bonos
        ...
        # _sync_bonos ya corrio en el create -- solo recalcular
        slip._compute_bono_salarial()
        slip._compute_gross()
        self.assertAlmostEqual(
            slip.bono_salarial_amount, 50000, delta=1,
            msg='bono_salarial_amount debe ser CRC50,000'
        )
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            ...
            msg=f'CCSS obrera debe calcularse sobre salario+bono. '
                f'Esperado: CRC{ccss_expected:,.2f}, obtenido: CRC{slip.ccss_employee:,.2f}'
        )
        bono.unlink()

    # -- TEST: Embargo respeta limite 25% neto (Art. 172 CT) -----
    ...
    def test_20_embargo_limite_25_pct(self):
        ...
        # Porcentaje valido (20%) -- debe crearse sin error
        ...
        # Porcentaje invalido (30%) -- debe fallar
    ...
    def test_21_paternity_daily_rate(self):
        ...
        self.assertAlmostEqual(
            ...
            msg=f'Paternidad 8 dias habiles = salario/30 x 8 = CRC{expected:,.2f}. '
                f'Obtenido: CRC{slip.paternity_amount:,.2f}'
        )
